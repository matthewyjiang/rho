...
pub(super) fn check_cancelled(context: &ToolContext) -> Result<(), ToolError> {
...
pub(super) fn workspace(context: &ToolContext) -> Result<&Workspace, ToolError> {
    context
        .workspace()
        .ok_or_else(|| ToolError::new(ToolErrorKind::Execution, WORKSPACE_REQUIRED))
}

pub(super) fn workspace_root(context: &ToolContext) -> Result<&Path, ToolError> {
    context
        .workspace_root()
        .ok_or_else(|| ToolError::new(ToolErrorKind::Execution, WORKSPACE_REQUIRED))
}
