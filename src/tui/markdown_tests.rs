...
#[test]
#[ignore]
fn measure_unicode_wrap_prefix_scans() {
    use std::{hint::black_box, time::Instant};

    fn legacy_wrap(segments: &[StyledSegment], width: usize) -> Vec<Line<'static>> {
        let text = segments
            .iter()
    ...
    let segments = markdown_inline_segments(&text);
    let iterations = 20;
    let started = Instant::now();
    for _ in 0..iterations {
        black_box(legacy_wrap(black_box(&segments), 17));
    }
    let legacy = started.elapsed();
    let started = Instant::now();
    for _ in 0..iterations {
        black_box(wrap_styled_segments(black_box(&segments), 17));
    }
    let optimized = started.elapsed();
    eprintln!(
        "legacy={legacy:?} optimized={optimized:?} speedup={:.2}x",
