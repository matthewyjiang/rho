...
const IMPLEMENTED_PROVIDERS: &[&str] = &["openai"];
...
fn available_models_from(catalog: &[ModelCatalogEntry], auth: &str) -> Vec<ModelCatalogEntry> {
    let mut models = catalog
        ...
        .filter(|entry| entry.auth_modes.contains(&auth))
        .collect::<Vec<_>>();
...
fn resolve_model_selection_from(
    ...
    if let Some((provider, model)) = input.split_once('/') {
        ...
        let from_catalog = available_models_from(catalog, auth)
            .iter()
...
mod tests {
    ...
    #[test]
    fn available_models_filters_to_implemented_providers_and_auth() {
