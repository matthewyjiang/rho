...
pub(super) fn intercept(
    ...
    if !request.messages.iter().any(|message| matches!(message.semantic(), SemanticMessage::User(blocks) if blocks.iter().any(|block| matches!(block, ContentBlock::Text(text) if text == "fixture boundary notification")))) {
        return None;
    }
    if request.messages.iter().any(|message| matches!(message, Message::ToolResult(result) if result.id.as_str() == "boundary-fixture")) {
        let delivered = request.messages.iter().any(|message| matches!(message.semantic(), SemanticMessage::User(blocks) if blocks.iter().any(|block| matches!(block, ContentBlock::Text(text) if text.contains("[process notification]")))));
