...
fn conversation_effort_changes_keep_prefix_and_pin_system_messages() {
    ...
    let top = apply(FABLE_5_1, &mut state, Some(effort("high")), &mut first);
    assert_eq!(top, Some(effort("high")));
    assert!(system_efforts(&first).is_empty());
    assert_eq!(beta_header(&first), None);

    let mut second = turn(2);
    let top = apply(FABLE_5_1, &mut state, Some(effort("low")), &mut second);
    assert_eq!(top, Some(effort("high")));
    ...
    let top = apply(FABLE_5_1, &mut state, Some(effort("low")), &mut third);
    assert_eq!(top, Some(effort("high")));
    assert_eq!(system_efforts(&third), ["low"]);
    assert_eq!(third[2].role, AnthropicRole::System);

    let mut fourth = turn(4);
    let top = apply(FABLE_5_1, &mut state, Some(effort("medium")), &mut fourth);
    assert_eq!(top, Some(effort("high")));
    assert_eq!(system_efforts(&fourth), ["low", "medium"]);

    let mut fifth = turn(5);
    let top = apply(FABLE_5_1, &mut state, Some(effort("high")), &mut fifth);
    assert_eq!(top, Some(effort("high")));
...
fn non_continuation_requests_use_top_level_current_effort() {
    ...
    apply(FABLE_5_1, &mut state, Some(effort("low")), &mut messages);

    let mut same_len = turn(1);
    let top = apply(FABLE_5_1, &mut state, Some(effort("high")), &mut same_len);
    assert_eq!(top, Some(effort("high")));
    assert!(system_efforts(&same_len).is_empty());

    let mut continued = turn(2);
    apply(FABLE_5_1, &mut state, Some(effort("high")), &mut continued);
    ...
    let top = apply(FABLE_5_1, &mut state, None, &mut off_messages);
    assert_eq!(top, None);
    assert!(system_efforts(&off_messages).is_empty());

    let mut unsupported = turn(1);
    let top = apply(FABLE_5, &mut state, Some(effort("low")), &mut unsupported);
