...
// Covers: missing credentials or an unsupported provider must not abort
// interactive startup; the TUI opens so the user can log in or switch.
// Owner: app bootstrap policy
#[test]
fn provider_setup_errors_are_nonfatal_for_interactive_startup() {
    for (case, error) in [
        (
            "missing credentials",
        ...
        assert!(is_interactive_startup_unavailable_error(&error), "{case}");
    }
}
