...
- **Permission mode**: `permission_mode` must be `bypass`, `auto`, `allow_edits`, `plan`, or `supervised`. Set it under Agent behavior in `/config`, or in config. `bypass` allows every capability. `auto` uses the same gate as `allow_edits` and classifies the rest. `allow_edits` allows in-workspace writes to git-tracked files and asks before new files and process execution. `plan` denies file writes and process execution. `supervised` asks before file writes and process execution. The change applies before the next turn and clears session approvals.
