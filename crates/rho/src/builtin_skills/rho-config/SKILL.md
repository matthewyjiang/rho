...
- **Cache miss notices**: under Context & limits in `/config`, or `display.cache_miss_notices` in config. Off by default. When on, a completed turn that re-billed a large uncached prompt (over 20K tokens or $0.10) inserts a transcript notice. `/info` always reports session cache hit rate and re-billed totals.
