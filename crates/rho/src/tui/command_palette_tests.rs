...
fn completing_goal_command_reveals_lifecycle_actions() {
    ...
    assert_eq!(app.input_ui.input, "/goal ");
    ...
    app.input_ui.input = "/goal release is published".into();
    app.input_ui.input_cursor = app.input_ui.input.chars().count();
    app.input_changed();
    assert!(!app.command_palette_visible());
}
...
fn goal_lifecycle_action_completion_replaces_placeholder() {
    ...
    app.input_ui.input_cursor = app.input_ui.input.chars().count();
    app.clamp_command_selection();
    app.input_ui.command_selection = 1;

    let clear = app.selected_command().unwrap();
    app.complete_command_choice(&clear);

    assert_eq!(app.input_ui.input, "/goal clear");
    assert_eq!(app.input_ui.input_cursor, "/goal clear".chars().count());
    assert_eq!(
        app.input_ui.input_submission_mode,
...
fn exact_template_match_precedes_builtin_prefix_match() {
    ...
    app.input_ui.input = "/mod argument".into();
    app.input_ui.input_cursor = 4;
...
fn template_completion_expands_pasted_arguments_and_clears_segments() {
    ...
    assert_eq!(app.input_ui.input, "Review this: alpha\nbeta ");
...
fn template_completion_marks_slash_prefixed_contents_as_prompt() {
    ...
    app.input_ui.input = "/review".into();
    app.input_ui.input_cursor = app.input_char_len();
    let choice = app.selected_command().unwrap();

    app.complete_command_choice(&choice);

    assert_eq!(app.input_ui.input, "/diff literally ");
    assert_eq!(
        app.input_ui.input_submission_mode,
        InputSubmissionMode::Prompt
    );
}
