...
fn completing_goal_command_reveals_lifecycle_actions() {
    ...
    app.input_ui.text = "/goal release is published".into();
    app.input_ui.cursor = app.input_ui.text.chars().count();
    app.input_changed();
    assert!(!app.command_palette_visible());
}
...
fn exact_template_match_precedes_builtin_prefix_match() {
    ...
    app.input_ui.text = "/mod argument".into();
    app.input_ui.cursor = 4;
