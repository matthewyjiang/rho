//! Who owns the full terminal: setup, in-place attach, or the session.
//!
//! This is stored state, not a query over sibling flags. Setup and attach both
//! replace session chrome. Only attach swallows input; setup still types into
//! its composer pickers.
...
impl App {
    ...
    /// composer. Attach consumes every event and reports whether it was a resize.
