...
> Permission-mode note: these live captures predate the current spawn policy. They used `--permission-mode dontAsk` with `--setting-sources project`. Production advisor one-shots still set `dontAsk` with no tools, but now pass an empty `--setting-sources` list. Delegated Auto and Allow edits set `dontAsk` only when tools are validated no-prompt names (not bare Bash, Edit, or Write) and Claude settings are not inherited; otherwise they refuse spawn. The NDJSON bodies remain valid stream fixtures regardless of the spawn permission mode used at capture.

| `live_success.ndjson` | Live capture, 2026-07-24, Claude Code `2.1.219` (Pro / claude.ai). Prompt: `reply exactly \`rho-claude-e2e-ok\``. Capture args predate the current spawn policy (`--setting-sources project` instead of today's empty dontAsk list): `claude -p --output-format stream-json --verbose --include-partial-messages --permission-mode dontAsk --tools "" --disallowedTools Task --setting-sources project --strict-mcp-config --max-turns 1`, prompt on stdin, isolated empty cwd (no project CLAUDE.md). Sanitized (not byte-for-byte): real `session_id` / message id / request id / envelope uuids replaced with stable fixture UUIDs of the same shape; cwd and `memory_paths` rewritten to `/tmp/rho-claude-fixture-*`; init `slash_commands` / `agents` / `skills` emptied (machine-specific capability/config lists); rate-limit `resetsAt` / `overageResetsAt` and assistant `timestamp` normalized to stable placeholders; no email, org id, or home-directory path retained. Message structure, partial deltas, rate-limit event shape, usage, and `modelUsage` kept; response text remains `rho-claude-e2e-ok`. |
| `live_tool_roundtrip.ndjson` | Live capture, 2026-07-24, Claude Code `2.1.219` (Pro / claude.ai). Isolated temp cwd with only `note.txt` containing `rho-tool-fixture-marker-42`. Prompt: read `note.txt` with the Read tool, then reply with exactly the file contents. Capture args predate the current spawn policy (`--setting-sources project` instead of today's empty dontAsk list): `claude -p --output-format stream-json --verbose --include-partial-messages --permission-mode dontAsk --tools Read --allowedTools Read --disallowedTools Task --setting-sources project --strict-mcp-config --max-turns 4`, prompt on stdin, no user/local setting sources, no MCP. Sanitized (not byte-for-byte): real `session_id` / message ids / tool ids / request ids / envelope uuids replaced with stable fixture ids of the same shape; cwd and `memory_paths` rewritten to `/tmp/rho-claude-fixture-*`; init `slash_commands` / `agents` / `skills` emptied; rate-limit `resetsAt` / `overageResetsAt` and timestamps normalized; no email, org id, or home-directory path retained. Message structure kept: tool_use Read + tool_result + later assistant text + terminal result (`num_turns=2`, final text `rho-tool-fixture-marker-42`). |
