...
> Permission-mode note: these live captures used `--permission-mode dontAsk`. Production advisor one-shots still set `dontAsk` with no tools and empty `--setting-sources`. Delegated Auto and Allow edits set `dontAsk` only when tools are bare names and Claude settings are not inherited; otherwise they refuse spawn. The NDJSON bodies remain valid stream fixtures regardless of the spawn permission mode used at capture.
