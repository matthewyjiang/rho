...
> Permission-mode note: these live captures used `--permission-mode dontAsk`. Production delegated Auto and Allow edits runs set `dontAsk`; advisor one-shots also set `dontAsk`. The NDJSON bodies remain valid stream fixtures regardless of the spawn permission mode used at capture.
