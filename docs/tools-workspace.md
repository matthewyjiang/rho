...
When the active model provider is xAI, Rho enables xAI's hosted `x_search` tool on every model turn. That tool searches X (x.com) posts, users, and threads server-side. It is separate from the client `web_search` tool, which still searches the open web through configured search backends. Because `x_search` is attached by the xAI provider request path, switching an existing session to xAI adds it on the next turn, and switching away removes it. Hosted X Search activity appears in the run stream as provider-native search events named `x_search`.
