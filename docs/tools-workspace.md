...
`read_file` and `fetch_content` share Rho's bounded document extractor. Along with UTF-8 text and source files, it extracts text-layer PDFs, DOCX documents, and XLSX, XLS, or ODS spreadsheets. Spreadsheet output uses bounded Markdown tables. PDF extraction accepts classic cross-reference files, preflights Flate stream expansion against a 64 MiB budget, and rejects object/xref streams or filters that cannot be bounded. Extraction warnings and truncation are included in tool output and metadata. Scanned PDFs without a text layer report a clear warning because OCR, archive recursion, PPTX, and native provider document parts are not included. Remote PDFs use the same pure-Rust extraction path instead of a placeholder.
