...
All stay optional. Outside Herdr, Rho skips host reporting. Without RTK, shell tools run commands unchanged. MCP stays inert until you add an enabled server under `[mcp.servers]` or a plugin package provides one. Agent Plugins load only from the explicit plugin roots.
...
- [Agent Plugins](/integrations/plugins) - package layout, discovery roots, skills and MCP components, failure isolation

Check setup from the interactive TUI with `/doctor`. Inspect MCP with `/mcp` or `rho mcp list`. Plugin load problems appear in both.
