...
- Permissions: both request read access only. Workspace-scoped searches work in every permission mode, including `plan`. Searching outside the workspace is denied in `plan` and asks first in `auto`, `allow_edits`, and `supervised`.
