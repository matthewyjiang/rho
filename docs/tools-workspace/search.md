...
- Path: `grep` `path` may be a directory or a single file. A file path searches that file only, including when a directory walk would have skipped it as ignored or hidden.
- Defaults: directory walks honor `.gitignore`, skip hidden files, and never follow symlinks. Pass `include_hidden` when you need dotfiles.
...
- Permissions: both request read access only. Workspace-scoped searches, plus the user's global `AGENTS.md`, skill trees, and agent definitions, work in every permission mode, including `plan`. Other searches outside the workspace are denied in `plan` and ask first in `auto`, `allow_edits`, and `supervised`.
