...
`grep` searches file contents with a regex. `glob` lists files whose paths match a pattern. Both run in-process and do not need `rg`, `fd`, or [RTK](/integrations/rtk). Prefer these tools over shell search for workspace inspection. `grep` content mode mints chainable `[path#TAG]` snapshots and match line numbers for the default `edit` format. Match text is search preview only.
