...
Queued images show a short preview above the composer text when the terminal supports graphics (Kitty or Ghostty), capped at 6 rows. Multiple consecutive images pack left-to-right at a shared height with a small gap, rather than stretching across the full width. Each preview keeps its `[image N: ...]` label on the row beneath. Document and pending extractions still use a full-width one-line label. Without graphics support, images keep the text label only.
