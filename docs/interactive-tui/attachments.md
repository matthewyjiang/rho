...
Queued images show a short preview above the composer text when the terminal supports graphics (Kitty or Ghostty), capped at 6 rows and fit to the composer width. Document and pending extractions still use a one-line label. Without graphics support, images keep the text label only.
