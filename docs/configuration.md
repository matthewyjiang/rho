...
Unknown keys in `config.toml` are ignored with a startup warning so typos stay visible. Values that Rho clamps or normalizes (for example `display.max_tool_output_lines` below 1, or an unsupported `web_search.provider`) also warn at load time. Prefer `/config` or a careful hand edit when you want durable settings; a full config save still rewrites the known schema and does not preserve comments.
