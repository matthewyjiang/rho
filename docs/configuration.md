...
- `auto` uses the same capability gate as `allow_edits`. A permission-classifier model reviews gated requests instead of opening the approval UI. It runs in two stages: a fast screen answers `allow` or `escalate` in one token, and only an escalation pays for a second, reasoned review that writes the final verdict. Both stages use the configured classifier reasoning level so the review can reuse the screen's prompt-cache prefix. Denied calls return a tool error and the run continues. After three consecutive or twenty total classifier denials, Rho escalates to the human approval prompt in the TUI or fails closed in headless runs; a human decision clears both counts. Auto requires a configured classifier model; choosing it from `/config` opens the model picker when none is set, starting interactive Auto without one opens the same picker, and headless `rho run` fails at startup without one. Escaping the startup picker falls back to Supervised so gated tools still ask a human.
