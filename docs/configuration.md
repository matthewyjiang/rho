...
- `allow_edits` lets the agent edit git-tracked files in the workspace without a prompt. After a new in-workspace file is allowed once this session, later edits to that path also skip the prompt. The user's global `~/.rho/AGENTS.md`, skill trees (`~/.rho/skills`, `~/.agents/skills`), and agent definitions (`~/.rho/agents`, `~/.agents/agents`) are treated the same way. Gitignored paths, other writes outside the workspace, process execution, and other reads outside the workspace still ask first. Workspace-scoped reads, network access, skills, and instruction discovery do not prompt. `allow-edits` is accepted as an alias.
