...
`agent_concurrency` is the maximum number of delegated agents that may execute at once, including background runs. It defaults to `10` and must stay between `1` and `64`. Change it from **Agent behavior** > **Concurrent agents** in `/config`, or set `behavior.agent_concurrency` in config. `/config` applies immediately: in-flight agents keep their slots, and queued agents start as capacity frees. `RHO_AGENT_CONCURRENCY` still overrides the file at process start; a live `/config` edit updates this session even when that env is set. Claude-cli runs also take a nested cap of 2 by default (`RHO_CLAUDE_AGENT_CONCURRENCY`), always `min(total, claude_cap)`.
