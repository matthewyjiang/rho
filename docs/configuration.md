...
`/info` includes compaction counts and decisions. For structured output, ask the agent to call `rho` with `action = "compaction"`. The report includes the current calibrated and local counts, provider request baseline, configured window, threshold, target, and the last idle and SDK checks with skip reasons. A check requesting compaction is not proof that it completed; the `compact` card shows the outcome. These diagnostics contain token counts and configuration, not conversation text.
