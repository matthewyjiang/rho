...
Inside the TUI, `/resume`, `/tree`, `/title`, and `/export` are unavailable.
Use `/copy` to copy an answer, or start Rho without the flag to resume saved work.

This is not a privacy or sandbox mode. Providers still receive requests, tools
can modify files, and hooks, logs, usage records, delegated agents, and workflows
can retain their own data. `--save` controls configuration overrides separately;
it does not turn session saving back on.
