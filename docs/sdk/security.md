...
The default-deny posture above applies to external SDK embedders. The shipped Rho CLI, automation mode, and interactive TUI enable unrestricted path resolution, while the selected [permission mode](/configuration#permission-modes) controls capability authority. `bypass` allows requests, `auto` allows the same writes as `allow_edits` and sends everything outside that gate to a configured permission-classifier model: new-file, untracked, and gitignored writes, writes or reads outside the workspace, process execution, and capability classes this version does not recognize. `allow_edits` allows in-workspace writes to git-tracked files plus workspace-scoped reads and the user's global `AGENTS.md`, skill trees, and agent definitions, and asks before other writes (including those global surfaces) and the rest, `plan` denies writes, process execution, and other reads outside the workspace, and `supervised` asks before those gated actions. Shell and `claude-cli` children are outside the path-scoped read gate, like any subprocess. Supervised or Allow edits non-interactive runs and headless Auto without a classifier model fail closed. Built-in tools still run with the current process's operating-system rights, so do not treat these controls as a sandbox. Hosts that need tighter controls must embed the SDK with an explicit workspace and policy.
