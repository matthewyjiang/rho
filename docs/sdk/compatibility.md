...
Nested provider-neutral message history preserves Rho's historical externally tagged enum representation, including legacy assistant messages, enriched assistants, and aborted assistants. Opaque provider context is data, not a public provider wire contract. Replay requires an exact identity unless a [known backend format permits cross-model replay](/sdk/providers#provider-native-replay-and-handoff).
