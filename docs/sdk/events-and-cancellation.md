...
Retryable physical provider request failures emit typed `ProviderRequestRetry` and, for 1.0 hosts, still dual-emit the legacy `ProviderActivity` kind `provider_request_retry`. Provider-native web search emits typed `WebSearch` and dual-emits legacy `ProviderActivity` kind `web_search`. Malformed responses still emit legacy `invalid_response_retry` immediately before `ProviderStreamReset`. New hosts should match the typed variants; the legacy activity kinds and `ProviderActivity` itself are deprecated and will be removed in the next major release.
