...
Retryable physical provider request failures emit typed `ProviderRequestRetry` and, for 1.0 hosts, still dual-emit the legacy `ProviderActivity` kind `provider_request_retry`. Provider-native search emits typed `WebSearch { name, detail }` (`name` is typically `web_search` or `x_search`) and dual-emits legacy `ProviderActivity` with the same `kind`. New hosts should match the typed variants; the legacy activity kinds and `ProviderActivity` itself are deprecated and will be removed in the next major release.
