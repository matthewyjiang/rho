...
Retryable physical provider request failures emit typed `ProviderRequestRetry` and, for 1.0 hosts, still dual-emit the legacy `ProviderActivity` kind `provider_request_retry`. Provider-native web search emits typed `WebSearch { detail }` and dual-emits legacy `ProviderActivity` kind `web_search`. Provider-native X search emits typed `XSearch { detail }` and dual-emits legacy `ProviderActivity` kind `x_search`. New hosts should match the typed variants; the legacy activity kinds and `ProviderActivity` itself are deprecated and will be removed in the next major release.
