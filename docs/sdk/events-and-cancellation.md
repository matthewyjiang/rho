...
Retryable physical provider request failures emit typed `ProviderRequestRetry`. Provider-native web search emits typed `WebSearch { detail }`. Other provider-native hosted tools (for example xAI `x_search` and `image_generation`) are carried on the provider stream via `ModelEvent::hosted_tool_activity` and lowered to typed `RunEvent::HostedToolActivity { name, detail }`.
