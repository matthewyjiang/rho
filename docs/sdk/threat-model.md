...
| File reads/writes | Traversal, absolute-path escape, symlink escape, race, or ambiguous normalization | Canonical workspace; lexical rejection; canonicalize existing targets; authorize at point of use | New targets and mutable parents retain TOCTOU risk; use platform-specific safe-open techniques |
| Process execution | Shell injection, executable substitution, inherited secrets, forked descendants, or unbounded output | Structured program/arguments; explicit cwd/env/executable/output policy; cancellation and process-group cleanup | Current capability request does not encode every process fact; tool and approval UI must add them |
| Network | SSRF, redirects, DNS rebinding, credential forwarding, large responses, or internal endpoint access | Explicit allowed host and authorization; validate scheme/port/redirect/destination; cap I/O | Host allowlist alone does not prevent all SSRF; enforce in HTTP adapter at every hop |
...
| Host approvals | Prompt spoofing, ambiguous command/path, stale decision, dropped responder, or accidental remembered grant | Structured request; allow-once/deny; response validation; cancellation | Request vocabulary may lack details; deny when context is insufficient and never infer remembered scope |
