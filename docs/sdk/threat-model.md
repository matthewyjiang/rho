...
| File reads/writes | Traversal, absolute-path escape, symlink escape, race, or ambiguous normalization | Canonical path scopes; explicit host opt-in for unrestricted paths; operation-specific resolution; scoped authorization; immediate revalidation; canonical execution path | Mutable filesystems retain residual TOCTOU risk; broad path access trusts host policy; use descriptor-relative safe-open or OS isolation for stronger guarantees |
