...
Choose **API Key** and paste the key when prompted. Rho stores it in the configured credential store and refreshes the hosted model list.
...
and choose the API key credential if more than one Ollama Cloud mode is stored.
...
3. Select a model the same way:

```text
/model ollama-cloud/<hosted-model>
```

`/logout` for the device mode clears Rho's session marker. It does not delete `~/.ollama/id_ed25519` or disconnect the key from ollama.com.
