...
Google may add, rename, or retire models, so refresh the list when a model is missing. Rho uses [models.dev](https://models.dev/) lifecycle metadata to hide exact model IDs marked as deprecated. If models.dev is unavailable or does not know an ID, Rho keeps the model returned by Google visible.
