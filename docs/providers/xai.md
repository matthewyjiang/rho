...
- Both `xai` and `xai-oauth` use xAI [server-side context compaction](https://docs.x.ai/developers/advanced-api-usage/context-compaction) (`POST /v1/responses/compact`) when automatic or manual compaction runs. The compact request body is only `model` plus full `input` (system messages included so the encrypted artifact can stand in for the prior window). After compact succeeds, Rho re-applies the host system prompt (base instructions, AGENTS.md, and the available-skills catalog) to replacement history so those instructions are not lost if the provider returns only a compaction marker. Tool schemas are not stored in history; they are re-advertised on every model turn. The encrypted item only replays on a compatible xAI Responses turn for the same provider identity and model.
