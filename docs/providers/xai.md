...
- Both auth modes also attach xAI's hosted `image_generation` tool on create turns unless `[xai] image_generation = false`. In the TUI, `/config` → **xAI** exposes the same toggle when xAI is the conversation provider or an xAI login is already available. Like `x_search`, it is a provider amenity outside the agent tool allowlist and disappears when the session leaves xAI. The model writes the image prompt and may generate or edit images in the same turn. Completed images arrive as `image_generation_call` items, stream as `HostedToolActivity { name: "image_generation", detail }` (the prompt), and are stored as assistant `ContentBlock::Image` plus a replay item so later xAI turns can edit them. Generated images can be large; they are persisted in the session and resent on follow-up xAI turns.
