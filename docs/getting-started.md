...
For local project work, see [development](/development). If authentication, model selection, or environment setup fails, run `/doctor` in the TUI; use `/info` to inspect the active selection and `/limits` to inspect supported usage windows (including last-observed Claude Code limits after a `claude-cli` run).
