...
| `/agents [create]` | With no argument, reload agent definitions and browse their descriptions, sources, runtime (`rho` or `claude-cli`), model policies, reasoning levels, tools, Claude config inheritance, prompt policies, and prompt previews. `/agents create [request]` starts the guided agent creator when the active agent has `skill`, `questionnaire`, `read_file`, `write`, and a platform shell tool. Select a reserved internal agent to configure its model. |
