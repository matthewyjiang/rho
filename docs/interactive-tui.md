...
| `/workflow` | Open the workflow list. Start a local workflow or plan in the background, open run status, reuse a saved plan, or press `d` to delete a plan/run. Keep chatting while a run continues; reopen `/workflow` for status. |
