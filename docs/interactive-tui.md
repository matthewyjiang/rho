...
| `/workflow` | Open the workflow list. Start a local workflow or plan in the background, watch a run on the DAG screen, reuse a saved plan, or press `d` to delete a plan/run. Keep chatting while a run continues; reopen `/workflow` and Enter a run to watch. |
