...
| `/compact` | Immediately summarize older conversation history to reduce future model context. This works even when auto compaction is disabled. The composer, scrollback, and paste stay usable while it runs; Enter queues a follow-up for after compaction finishes. Press `esc` to cancel. |
