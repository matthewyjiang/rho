...
Short confirmations and login choices use an inline list instead of a filterable picker. The arrow marks the focused option, with its label in bold and its shortcut beside it. Explanations wrap beneath each label. Use `Up`/`Down` or `Left`/`Right` to move, `Enter` or `Space` to choose, or press an option's shortcut to choose it immediately. Unavailable options are dimmed and skipped. `Esc` cancels or returns to the parent picker.
