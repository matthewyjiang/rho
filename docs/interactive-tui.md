...
| `/compact` | Immediately summarize older conversation history to reduce future model context. This works even when auto compaction is disabled. Auto compact runs the same job before a turn when the context is over the threshold. Both show a compact card; the composer stays usable. Press `esc` to cancel. |
