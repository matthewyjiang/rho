...
| `/info` | Show the running Rho version, provider, model, reasoning level, permission mode, advisor mode, session usage (including cache hit rate and re-billed cache misses), and external runtime status (including Claude Code ownership). |
