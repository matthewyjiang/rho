...
| `/login [provider]` | Log in with a provider, the Claude Code runtime, or the Cursor Agent runtime. No args opens a picker (Claude Code and Cursor Agent CLI are under **Anthropic**; Cursor is also a top-level row); direct args target a single [provider](/authentication-and-models#providers), `/login claude-code`, or `/login cursor`. |
