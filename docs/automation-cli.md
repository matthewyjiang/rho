...
| `rho doctor [--json]` | Run the same setup diagnostics as the interactive `/doctor` overlay and print a text report (or one JSON document). Exits with status 1 when any check fails; warnings exit 0. |
